...
def test_standard_svd():
    ...
    file_coeffs, file_dynamics = utils_spod.coeffs_and_reconstruction(
        data=data, results_dir=results_dir, time_idx='all', svd=True, comm=comm)
    T_ = 12.5;     tol = 1e-10
    f_, f_idx = spod.find_nearest_freq(freq_req=1/T_, freq=spod.freq)
    if comm.rank == 0:
        modes_at_freq = spod.get_modes_at_freq(freq_idx=f_idx)
...
def test_standard_inv():
    ...
    results_dir = spod.savedir_sim
    file_coeffs, file_dynamics = spod.coeffs_and_reconstruction(
        data=data, results_dir=results_dir,
        time_idx='all', svd=False, comm=comm)
    T_ = 12.5;     tol = 1e-10
    f_, f_idx = spod.find_nearest_freq(freq_req=1/T_, freq=spod.freq)
    if comm.rank == 0:
        modes_at_freq = spod.get_modes_at_freq(freq_idx=f_idx)
...
def test_standard_freq():
    ...
    if comm.rank == 0:
        ...
        coeffs = np.load(file_coeffs)
        recons = np.load(file_dynamics)
        recons_cl = np.load(file_dynamics_cl)
...
def test_standard_normalize():
    ...
    spod = SPOD_analysis.fit(data=data, nt=nt)
    # latent_space = spod.transform(
    #     data=data, nt=nt, rec_idx='all', svd=False,
    #     tol=1e-10, T_lb=0.5, T_ub=1.1)
    results_dir = spod.savedir_sim
    file_coeffs, file_dynamics = spod.coeffs_and_reconstruction(
        data=data, results_dir=results_dir, time_idx='all',
        tol=1e-10, svd=False, T_lb=0.5, T_ub=1.1, comm=comm)
    ...
    if comm.rank == 0:
        ...
        assert((np.real(np.max(recons))<4.474232181561473+tol2) & \

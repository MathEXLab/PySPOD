...
class SPOD_low_storage(SPOD_base):
	...
	def fit(self, X):
		...
		using RAM storage to reduce the amount of I/O and disk storage (for small
		datasets / large RAM memory machines).
		...
		self._nx = np.size(X[0,...,0]) # dimension of single snapshot as a column
		self._xdim = self._dim[1:-1]
		self._nv = self._params['nv']
		...
		if 'isreal'.lower() in self._params:
			...
			for ti in range(0,self._nt):
				isrealx = np.isreal(self._X[ti]).all()
				if not isrealx:
					break
		...
		gb_vram_required = self._n_freq * self._nx * self._nv * self._n_blocks * sys.getsizeof(complex()) * BYTE_TO_GB
		...
		Q_hat = np.zeros([self._n_freq,self._nx*self.nv,self._n_blocks], dtype='complex_')
		Q_blk = np.zeros([n_DFT,int(self._nx*self._nv)], dtype='complex_')
		if blocks_present:
			...
			for iBlk in range(0,self._n_blocks):
				...
				print('block '+str(iBlk+1)+'/'+str(self._n_blocks)+' ('+str(timeIdx[0])+':'+str(timeIdx[-1])+')')
		...
		self._modes = np.zeros([self._n_freq,self._nx*self._nv,n_modes_save], dtype='complex_')
		...
		self._modes = np.reshape(self._modes,(self._n_freq,)+self._dim[1:]+(n_modes_save,))
